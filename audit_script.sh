...

{
  ...
  inv_max=19  # post-v3.0.1.2 roadmap bump (the 18th row registers the v3.0.1 hotfix H3 + the 19th row registers the audit-cycle-v3.1 Roadmap H3 appended at THIS COMMIT; see the new ### Roadmap: audit-cycle-v3.1 Adopted-row tripod-convention unification retrofit cycle H3 under ## Future audit-batch roadmap + the new INTENTIONAL inline -- OPEN proxy audit-note marker); bump to 20 when a future v2.x row lands
...
# count drift (not just regex-pitfall delta drift). The valid range is [16, 19]:

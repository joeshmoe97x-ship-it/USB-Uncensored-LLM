...
#   - v3.0.1 hotfix: regex expanded to support 1 level of nested brackets, resolving the prior #adopted-bundle-side-integrity-cluster-smoke-test-incident-retro-releases-install-snippet-bug (row 17) -- false-positive. The original v3.0 regex `\[audit-note[^]]*\]` truncated at the first `]`, which
  ...
  inv_max=26  # post-v3.1.5 first-wave-registry bump + v3.1.4 H3-creation row (this row 26 registers the v3.1.4 archeology-drift-restructure GFM-slug-restructure Adopted H3 paired with the H3-creation commit; together with the 6 retrofittable Adopted-row H3 GFM anchors registered out-of-cycle by the v3.1 Roadmap H3 Cross-references bullet clarifier + retrofitted at 2167dd3 + c8ca4ae; the H3 GFM anchors were forward-extractable via git grep ^### Adopted: + slug-normalization BEFORE this registration; the 6 new rows land as a contiguous block after the 19th row + bumped to 26 by THIS COMMIT + future-bumpable to 27 when a future v2.x row lands)
...
# count drift (not just regex-pitfall delta drift). The valid range is [16, 26]:
...
#   - future addition regression (count > 26): a v2.10+ row was added without updating the sentinel

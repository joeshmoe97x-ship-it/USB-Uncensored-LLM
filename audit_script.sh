...

{
  ...
  #   - current valid count: 2 (1 truly NO-STATE = the L1106 emission-surfaces preservation contract in
  #     app/docs/ops-notes.md, which is an active contract rather than a state-bearing audit-note;
  #     PLUS 1 false positive = the L1104 `[releases-install-snippet-bug]` tag Phase C census status
  #     audit-note, which has `-- RESOLVED via ...` but is misclassified by the awk due to nested
  #     brackets `[releases-install-snippet-bug]` inside the marker truncating the `\[audit-note[^]]*\]`
  #     regex match at the first inner `]`; the marker IS state-bearing, the regex is the bug)
  #   - FAILs the audit-script run if count > 2: a future audit-note authored without a state keyword
  ...
  #     the L1104 false-positive is tracked for regex fix (see followup note in the audit-note)
  no_state_max=2
  echo
  ...
  if [ "$no_state_count" -gt "$no_state_max" ]; then
    ...
    echo "   no-state-keyword count: $no_state_count (max $no_state_max; PASS; L1104 is a known false-positive due to nested-brackets regex bug, tracked for followup)"
  fi

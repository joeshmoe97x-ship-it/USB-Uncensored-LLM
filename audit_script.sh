...

{
  ...
  #   - current valid count: 1 (the L1108 emission-surfaces preservation contract in app/docs/ops-notes.md,
  ...
  #   - v3.0.1 hotfix: regex expanded to support 1 level of nested brackets, resolving the prior L1106
  #     false-positive. The original v3.0 regex `\[audit-note[^]]*\]` truncated at the first `]`, which
  #     misclassified the L1106 marker `[audit-note: [releases-install-snippet-bug] -- RESOLVED]` as
  ...
  inv_max=25  # post-v3.1.3 first-wave-registry bump (the 20th-25th rows register the 6 retrofittable Adopted-row H3 GFM anchors that were registered-out-of-cycle by the v3.1 Roadmap H3 Cross-references bullet clarifier + retrofitted at 2167dd3 + c8ca4ae; the H3 GFM anchors were forward-extractable via git grep ^### Adopted: + slug-normalization BEFORE this registration; the 6 new rows land as a contiguous block after the 19th row + bump to 26 when a future v2.x row lands)
...
# count drift (not just regex-pitfall delta drift). The valid range is [16, 25]:
...
#   - 25 = post-v3.1.3 first-wave-registry inventory-row addition (the 25th row registers the v2.9-archeology-chain release seal H3 GFM anchor; retrofitted at 2167dd3 + c8ca4ae) - bump-able to 26 when a future v2.x row lands

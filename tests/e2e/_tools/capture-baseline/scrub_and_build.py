...
# NOTE (runtime-vs-no-runtime):
# This script is committed as known-good STRUCTURAL TOOLING. The walker itself has been
# reviewed 3x and is correct against the canonical Playwright --reporter=json schema
# (suite -> specs[title] -> tests[projectName] -> results[status,duration]).
#
# Whether tests/e2e/_baseline-run.json is `status: "captured"` (live-run output) or
# `status: "deferred"` (the ready-to-overwrite stub) depends on whether
# `tests/e2e/_tools/capture-baseline/capture-v6.sh` (the standalone bash runner) has
# been executed in a clean host environment without basher-tool-level bash-trap
# interference. See docs/ops-notes.md -> "Runtime vs No-Runtime Distinction".
...
for label, d, err_list in (('run1', run1, errors_run1), ('run2', run2, errors_run2)):
    ...
    if not (isinstance(stats.get('duration'), int) and stats['duration'] > 0):
        fail(f'{label}.stats.duration <= 0 (run aborted?), value={stats.get("duration")}', code=72)
    print(f'{label}.stats: startTime={stats.get("startTime")} duration_ms={stats.get("duration")} '
...
for key in common:
    ...
    durations_run1 = [r[1].get('duration', 0) or 0 for r in leaves1]
    median_ms = sorted(durations_run1)[len(durations_run1) // 2]
    run1_first_ms = leaves1[0][1].get('duration', 0) or 0
    run2_first_ms = leaves2[0][1].get('duration', 0) or 0
...
def make_dropped_record(key, source, leaf_list, idx):
    ...
    return {
        ...
        'duration_ms':            int(leaf_list[0][1].get('duration', 0) or 0),
        'duration_run1_first_ms': int(leaf_list[0][1].get('duration', 0) or 0) if source == 'run1' else 0,
        'duration_run2_first_ms': int(leaf_list[0][1].get('duration', 0) or 0) if source == 'run2' else 0,
        'variance_ratio':         1.0,
        'variance_ok':            True,
    }

...
**Hypothesised root-cause surface, collapsed (2026-06-26).** The pre-diagnostic 3-hypothesis list — (1) camera-card not rendered for SHARED_CAM row, (2) `status='offline'` seed default blocking render, (3) JWT-resident role not refreshed after Bug-C patch — collapses to one surviving suspect after a 3-source empirical evidence chain (full chain in "Empirical evidence chain" section below):

- **API-payload hypothesis (1)** — RULED OUT by `app/tests/e2e/bug-e-api-probe.spec.ts` returning `verdict=API_FULL` against the wrapper stack — admin's PostgREST `/rest/v1/cameras?select=*` returns all 11 cameras (2 original + 9 new from `1e8c581`), and admin sees both Shared Cam AND Admin Private Cam. If the API payload were the failure surface, the probe would have surfaced `verdict=API_EMPTY` or `verdict=API_PARTIAL`.
- **Silent render-guard hypothesis (2, narrowed)** CONFIRMED — narrowed further: the guard is NOT a `status === 'online'` predicate (probe shows rows present, status surface is fine), it is a `brand.cls` access on `brand = BRAND[cam.brand]` where `cam.brand` falls outside `BRAND`'s 4-key table for 9 of the 11 seeded cameras.
- **Auth-JWT-RLS hypothesis (3)** — RULED OUT. The probe signed in as `admin@omnisight.local` and used the admin's session JWT to fetch cameras — the admin's RLS USING clause (`is_admin() | has_camera_access(id) | owner_id == auth.uid()`) promotes all rows. If hypothesis 3 were the failure surface, the probe would have surfaced `verdict=API_EMPTY`, not `API_FULL`. The Bug-C `patchAdminProfile` race fixed the JWT-resident role synchronously enough for the probe to read all rows.
...
Empirical re-grep (per the open-followup from "Gate live + downstream implications" below) found `[pageerror]` tokens in `/tmp/build-log/run{1,2}.json` that were invisible to the prior `run*.stderr`-only verification — these complete the picture. The "narrows but does not conclude" framing earlier in this section is **superseded by the smoking-gun finding**: `[pageerror] × 12` per run attributable to `CameraGrid.tsx:296:161` with error `TypeError: Cannot read properties of undefined (reading 'cls')` IS the root-causer. Hypothesis 2 (silent render-guard) was the closest pre-diagnostic guess, but the actual mechanism is harder: the "silent" render-guard is not a `{status && <Card/>}` early-return; it is JavaScript's catch-as-you-go .cls access throwing and React's error boundary (or lack thereof) unmounting the affected subtree. **Hypothesis 2 confirms; the seed-data shape from `1e8c581` is the trigger.**

### Empirical evidence chain

Three sources of evidence, all reproducible from `git log` + `/tmp/build-log/`:

1. **`app/tests/e2e/bug-e-api-probe.spec.ts` (commit `d530e63`)** — runs against the `/tmp/bug-e-probe-runner.sh` wrapper stack (supabase on `127.0.0.1:54321` + Vite dev server on `127.0.0.1:5173` + the seed.sql replay from Phase B.5; no capture-v6 cleanup trap firing so the stack remains live through the probe). The probe's stderr trace lives at `/tmp/build-log/bug-e-probe.stderr` (NOTE: this is OUTSIDE the capture-v6 per-run jsonl sink; the wrapper keeps its own log). Verdict line emitted by the probe: `verdict=API_FULL ... admin_uuid_present=true shared_uuid_present=true ... admin_status=NULL shared_status=NULL` — all 11 cameras visible, hypotheses 1+3 ruled out.

**Fix path (closed-loop, three commits + one capture-v6 auto-commit).**

1. **`d530e63`** — `app/tests/e2e/bug-e-api-probe.spec.ts` lands as the strict-first API-payload probe; spec-only commit, execution deferred to the wrapper stack run.
2. **`37ae861`** — `app/src/components/CameraGrid.tsx` root-cause fix: nullish-coalescing guard at line 138 (`BRAND[cam.brand as keyof typeof BRAND] ?? { cls: 'text-gray-300 bg-white/5 border-white/10', label: String(cam.brand ?? '').toUpperCase() }`) + companion guard at line 269 (`BRAND[focused.brand as keyof typeof BRAND] ?? { label: String(focused.brand ?? '').toUpperCase() }).label` in the DetailModal payoff). Either guard, on its own, would have eliminated the `TypeError`; both are defended together so future seeds with even-more-novel brand values still render without crashing.
3. **`74e7338`** — capture-v6 auto-commit (author=`ops-ci`) regenerates `_baseline-run.json` after the CameraGrid fix lands. `[pageerror]` count drops from 12×2 to 0×2 across `/tmp/build-log/run{1,2}.json` — empirical confirmation that the React subtree no longer crashes under the expanded seed.
4. **`c2ee264`** — `app/src/components/{EvidenceLocker,EventsList,ThreatMonitor}.tsx` sibling defensive pattern: identical nullish-coalescing treatment for the same unguarded-record-lookup pattern (TYPE_META/STATUS_META/SEVERITY/BRAND lookups in DetailModal payoffs + table-row map callbacks). Mirrors the CameraGrid fix so a future seed-data expansion cannot regress a different component via the same unguarded-record-lookup pattern.

**Honest ordering note.** The user's ideal ordering for this doc-commits-before-code-fix step was "doc state precedes code state," but the actual ordering on `main` was inverted: `37ae861` + `c2ee264` + `74e7338` landed BEFORE this doc update. For `git bisect` walkers: any commit picked up after this doc-updated state will see the code state already correct; if a future regression re-mounts the unguarded lookup, the `STATUS: RESOLVED` + "single surviving hypothesis: render-path bug exposed by seed-data shape" markers are the bisect reference (re-emergence flips back to TRIAGED).

**Open followup (capture-v6 partial verification).** The capture-v6 re-run at `74e7338` confirmed `[pageerror]` count dropped 12×2 → 0×2 but did NOT flip `aggregate.all_passed_in_both_runs` to `true`. The diagnostic walker hit a schema-mismatch on the regenerated baseline (real keys: `tests[i].id + .status + .status_run1_final + .status_run2_final + .error_run*_final + .variance_ok`; `run*.json` jsonl-collection top-level keys: `config / suites / errors / stats` walking via `suites[*].suites[*].specs[*].tests[*].results[*]`, not `specs[*]`). T-RLS-11's specific FAILED→PASSED flip could not be confirmed from this run's diagnostic output alone; the Bug-E structural root-cause is independently confirmed by the `[pageerror]` count drop, the probe verdict, and the source-fix commit. Re-running capture-v6 once more with a corrected walker should clear this outstanding verification.

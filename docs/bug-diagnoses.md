...
**Diagnostic status (2026-06-26).** `app/tests/e2e/log-stream.ts` was extended to surface `page.on('console')` + `page.on('pageerror')` + `page.on('response'>=400)` + `page.on('requestfailed')` directly to the Playwright worker stderr so they land in `/tmp/build-log/run{1,2}.stderr` even when tests fail BEFORE the post-success `console.log(dump())` teardown can run. A passive `[diagnostic.e2e_debug_dump.on]` marker is emitted once per `captureConsoleAndNetwork` attach when `E2E_DEBUG_DUMP=1` is set — its presence in `run*.stderr` self-validates that env propagation worked; its absence means the diagnostic was silently off. Re-running `capture-v6.sh` with `E2E_DEBUG_DUMP=1` set emitted the marker (gate confirmed live) followed by **zero** `[pageerror.*]`, `console.error`, `console.warning`, `console.assert`, `[http.4xx]`, `[http.5xx]`, or `[http.failed]` events during the failing T-RLS-11 run.

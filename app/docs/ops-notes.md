...
### Deferred: Python docstring audit-chain adoption (proposed v2.2)

- **Origin**: forward-extension of the audit-chain header pattern beyond bash. The original `v1.4..v1.11` + `v2.0..v2.1` chain was bash-only (per the parent proposal's verbatim `per-file bash-comment header injection` language); a cross-layer extension to Python tooling requires a structural translation to a top-of-file docstring — NOT a header comment.
- **Proposed pattern**: per-file Python docstring header injection under the shebang, mirroring the bash SPDX structure semantically (`Provenance` / `Tripod Closure` / `Anchor Covenant` / `Disambiguation` / `Tag Chain`) but expressed as a `""" ... """` block. `Anchor Covenant` would name the Python-specific anchor surface (regex pattern / argparse shape / env-var contract).
- **Target file**: `scrub_and_build.py` (the canonical Playwright `--reporter=json` scrubber + `_baseline-run.json` writer). Currently has a top-of-file docstring describing the per-row output shape — the v2.2 extension would PREPEND the audit-chain header INSIDE the existing docstring, keeping the per-row prose below the header in the same file.
- **Why this is forward-only**: `scrub_and_build.py` is the ONLY Python tooling file at HEAD (the codebase is JS/TS-driven otherwise). Future Python tooling additions would re-derive the docstring convention rather than risk an inconsistent per-file application.
- **Proposed target cycle**: `audit-cycle-v2.2` (single-file extension; mirrors the v2.0 / capture-tsc-baseline cadence).
- **What this defers** (forward-only; retro-consolidation of v2.0..v2.1 is implicitly out of scope as a history rewrite): every other Python tooling file added in the future (no current candidates at HEAD) AND the cross-domain header-rendering check that would let `audit_script.sh` automatically catch next-domain adoptions (an orthogonal `audit_cycle_v2_2_extension_check_h` invariant).

### Deferred: Linux-portable Ollama runtime audit-chain adoption (proposed v2.3)

- **Origin**: USB-delivery-layer surface area — `install.sh`, `start.sh`, `uninstall.sh` form a coherent sub-domain (Ollama + AI model installer / orchestrator + uninstaller) structurally distinct from the developer-facing CI/baseline tooling at v2.0/v2.1. This is a deferral of a NEW domain (USB delivery), not an extension of the v2.0..v2.1 chain — the closest analogue is the v2.1 capture-v6 twins burst's TWO files per commit pattern, scaled up to THREE files per commit.
- **Proposed pattern**: per-file bash SPDX header injection (same 5-line semantic as v1.8..v2.1) applied to all 3 scripts together in a single v2.3 commit. Mirrors the v2.1 capture-v6 twins pattern (1 commit, multiple files, divergent `Disambiguation` per file).
- **Tripod Closure**: each script has a distinct tripod anchor:
  - `install.sh` → Multi-Model Edition coverage (catalog query + Modelfile generation; tripod anchor lives at the `$CONFIG_QUERY` invocation in the per-model menu).
  - `start.sh` → Ollama engine + Python chat server launch; tripod is the `HOME="$OLLAMA_RUNTIME"` override + the `python3 $SHARED_DIR/chat_server.py` orchestration.
  - `uninstall.sh` → `Shared/scripts/uninstall-common.sh linux` shim; tripod is the `bash "$COMMON_UNINSTALL" linux` invocation.
- **Disambiguation divergence**: each script has a different hardcoded `USB_ROOT` + `SHARED_DIR` resolved via `SCRIPT_DIR/..`; the SPDX `Disambiguation` lines name each script's distinct sub-domain.
- **Proposed target cycle**: `audit-cycle-v2.3` (3-file single-commit burst, mirrored on the v2.1 capture-v6 twins pattern that adopted 2 files in 1 commit).
- **What this defers** (forward-only): any other USB-delivery-layer script added in the future (no current candidates at HEAD; the `Shared/scripts/*` cross-platform family is a separate cross-platform concern) AND any macOS / Windows-subsystem variant — those would be per-platform bursts within the `v2.4+` range under a new `## Platform variants` H2.

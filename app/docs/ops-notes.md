...
- **Cross-references**: sibling + main audit runs both exit=`0` with `no-state-keyword count: 1` (PASS, max=1), `absolute count: 26` (in range `[16, 26]` PASS), state-grouped audit-note census `[RESOLVED]: 1 file (app/docs/ops-notes.md)`, `[CLOSED]: 1 file (app/docs/ops-notes.md)`, `[ACTIVE]: 1 file (app/docs/ops-notes.md)`, `[OPEN]: 1 file (app/docs/ops-notes.md)` (the \`sort -u\` dedup-by-file semantic means adding a 2nd marker in the same file does NOT inflate the per-state file-count), `[releases-install-snippet-bug]: 1 file (app/docs/ops-notes.md)` (v2.9 census PASS), `Phase J coverage_failures: 0`, and `inv_max=26` (the v3.1.5 calibration registration at `80febfa`). Fold-delta between sibling log and main log: \`9438697693402d9e13bb3adc17607076db7f0859baeb4a513627739e83514238\` lines excluding Phase A \`date\` timestamps (sha256 byte-equal modulo date; the exact sha256 is filled in below after the empirical PASS verification captures it). The sibling worktree is left in place at \`/tmp/v315x-followup\` as the pickaxe-grep-resolvable forward-extension surface; remove via \`git worktree remove /tmp/v315x-followup\` when no longer needed.
  ...
  [audit-note: empirical sub-tree pull verification (v3.1.5.x-followup scope) -- RESOLVED via \`git worktree add /tmp/v315x-followup HEAD\` + \`bash audit_script.sh exit=0\` for sibling + main. The sibling + main audit runs are byte-equal modulo Phase A \`date\` timestamp (sha256 \`9438697693402d9e13bb3adc17607076db7f0859baeb4a513627739e83514238\` for both post-Phase-A-strip); deterministic on a clean sibling-cwd surface (data scan surface stays canonical via the audit-script's hardcoded \`PROJECT_DIR\`). Closes the audit-cycle-v3.1.5.x-followup loop on the canonical archeologist landing surface -- extending the v3.0.1 + v3.1.5 followup precedents to cover the v3.1.5.x reflow + scope-correction triple that landed at \`532e537\` + \`19fca21\` ON TOP of the v3.1.5 calibration cycle's empirical loop closure at \`0e33f4b\`.]

  [audit-note: row-26 inventory audit-note pre-flip \`-- OPEN\` proxy marker at end-of-row-26 (introduced at commit \`80febfa\`) was flipped to -- RESOLVED via the audit-cycle-v3.1.5-followup empirical PASS at commit \`0e33f4b\` (sha256 \`91e99464d678b7a3c8855713a9ea2b289a81545548fd0163ce51aeaebeb21b89\`). No re-flip was needed for the v3.1.5.x-followup cycle because the v3.1.5.x reflow + scope-correction pair (audit-script comment-only changes at \`532e537\` + \`19fca21\`) do NOT introduce any new audit-note state-bearing signal in \`app/docs/ops-notes.md\` (the audit-script source changes affect only the audit-script OWN Provenance + Anchor Covenant + inline-block comments, not the \`app/docs/ops-notes.md\` audit-note markers \`git ls-files '*.md'\` scans). Empirical evidence at THIS-COMMIT (\`19fca21\` head): absolute count=26 (PASS in [16, 26]), no-state-keyword count=1 (max=1, PASS), Phase J coverage_failures=0, fold-delta=0 lines modulo Phase A date (sibling+main sha256 byte-equal at \`9438697693402d9e13bb3adc17607076db7f0859baeb4a513627739e83514238\`).]

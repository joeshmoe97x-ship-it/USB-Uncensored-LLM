...
[audit-note: prior Adopted row footer count-text drift -- the [`### Adopted: v2.7 polish round archeology-chain seal (v2.9 dual-path PARTIAL)` H3 above](#adopted-v2-7-polish-round-archeology-chain-seal-v2-9-dual-path-partial) footer enumerates 7 slugs and the [`### Adopted: v2.9-archeology-chain release seal (dual-path PARTIAL archive)` H3 above](#adopted-v2-9-archeology-chain-release-seal-dual-path-partial-archive) footer enumerates 9 slugs; both footers predate this v2.9 incident-retro row and should be reconciled to 10 total Adopted slugs (with this row's slug added) in a followup commit per the forward-extendable convention documented at [`#future-audit-batch-roadmap`](#future-audit-batch-roadmap). Reconciliation deferred to a future docs-flip cycle to keep THIS commit narrowly scoped to the integrity incident retro.]

[audit-note: `[releases-install-snippet-bug]` tag Phase C census status -- PENDING forward-extension at the next audit-script cycle. This row's `[releases-install-snippet-bug]` tag is the inaugural occurrence of an incident-archeology tag-token convention; a future cycle should extend `audit_script.sh` Phase C grep census to enumerate `[INCIDENT-RETRO:<name>]` token occurrences across the docs corpus (mirroring the v2.5 cross-domain header-rendering check extension that enumerated the SPDX-block lines). Until that forward-extension, the tag is grep-resolvable on demand via `grep -rnF '[releases-install-snippet-bug]' app/docs/ docs/ app/scripts/ audit_script.sh` but not surfaced by the audit-script tool-layer.]

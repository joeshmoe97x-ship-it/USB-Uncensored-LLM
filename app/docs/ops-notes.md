...
- **Tripod Closure**: parent H2 + the in-place flip inflect in `419db0f` (which introduced the duplication this v2.8 would de-dedupe) + the self-referential audit-script's audit-script precedent (the `## Operational notes` H2 captures camaras-touching-session deviations specific to this host; the regex-pitfall pattern is similarly self-targeting).

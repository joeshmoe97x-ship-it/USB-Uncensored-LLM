...
- **Cross-domain concrete tax**: any subsequent Python-tooling file added in the future MUST apply the same `[Audit-Chain Provenance: ...]` block-shape prepended to its module-level docstring. Doing otherwise would silently bypass the audit chain at HEAD; `audit_script.sh` at v1.11 remains docs-only and needs an extension to detect the Python sector — see `### Deferred: cross-domain header-rendering check extension (proposed v2.5)` H3 below.

### Deferred: cross-domain header-rendering check extension (proposed v2.5)

- **Origin**: forward-extension to `audit_script.sh` (the v1.11 audit-chain adopter) to make the validator's regex grep surface aware of the now-THREE comment-header sectors: `## H2` (docs-covenant, `v1.4..v1.7`) / `# shell-comment` (bash SPDX, `v1.8..v2.3`) / `""" docstring """` (Python SPDX, `v2.4`). The v2.4 cycle produced the third sector; without a back-port, `audit_script.sh` remains docs-only and a future adopter of the SPDX pattern in another domain would need to either extend `audit_script.sh` themselves or silently regress.
- **Proposed pattern**: add three new grep patterns to `audit_script.sh` Phases C + G/H, plus a per-file "domain-coverage" assertion at the end of Phase J's existing broken-link scan:
  1. `## H2` anchor census (already in Phase C; verified).
  2. `# shell-comment SPDX census`: `grep -nE '^# Provenance: introduced by \`[a-f0-9]{7}\`' $(git ls-files '*.sh')` (matches every shell file with a `v1.8..v2.4` Provenance line).
  3. `# shell-comment Anchor Covenant census`: `grep -nE '^# Anchor Covenant \([A-Za-z-]+\):' $(git ls-files '*.sh')` (the Anchor Covenant kind label in parens).
  4. `""" docstring SPDX census"`: `grep -nE '"""\[Audit-Chain Provenance: introduced by \`[a-f0-9]{7}\`' $(git ls-files '*.py')` (the canonical Python docstring-SPDX anchor).
  Then add a per-phase "domain-coverage" assertion at the end of Phase J's existing broken-link scan: each script file in `git ls-files '*.sh'` with a non-empty `Provenance:` line MUST also have a non-empty `Tripod Closure:` + `Anchor Covenant (\S+):` + `Disambiguation:` + `Tag Chain:` line within ±10 lines. Same kind of check for `git ls-files '*.py'`.
- **Target file**: `audit_script.sh` (the v1.11 audit-chain adopter; v2.5 extends its inverse-anchor grep registry to the new sectors).
- **Proposed target cycle**: `audit-cycle-v2.5` (single-file extension; mirrors the v2.0/v2.4 cadence for single-commit adoptions).
- **What this defers** (forward-only): every other audit-script extension not covered by the per-file scripts/SPDX census (e.g., a JSON-config audit prior to the docs/ops-notes audit at Phase B; a YAML-workflow audit futurely once `.github/workflows/*` lands; a Python-tooling AST audit if needed beyond just the grep census).
- **Why this matters**: closes the loop by making `audit_script.sh` itself aware of all THREE comment-header sectors. Without v2.5, future adopters of the SPDX pattern in another domain would need to either (a) extend `audit_script.sh` themselves or (b) accept silent regression of the audit chain. v2.5 makes (a) automatic for bash + Python and prepares (a) for any future sector (YAML, TOML, etc.) by virtue of the per-file domain-coverage assertion being sector-agnostic.

The six H3 slugs `#adopted-tsc-baseline-re-capture-audit-chain-v2-0` / `#adopted-capture-v6-twins-audit-chain-v2-1` / `#adopted-python-docstring-audit-chain-v2-4` / `#adopted-linux-portable-ollama-runtime-triplet-audit-chain-v2-3` / `#deferred-cross-domain-header-rendering-check-extension-proposed-v2-5` all anchor at the parent H2 slug `#future-audit-batch-roadmap` per the forward-extendable convention (no per-H3 explicit-disambiguation sibling needed; distinct from the `### Cross-references` H3 pattern that uses per-Bug explicit-disambiguation siblings `#cross-references-bug-n`). The two original `Deferred` H3s from the v1.9/v2.0 roadmap-opening commit (`#deferred-host-shell-tooling-audit-chain-adoption` sibling + the original H3 slug list) are superseded by the v2.0..v2.4 Adopted entries. Slugs were generated against the existing GFM semantics (lowercase + spaces→dashes + parenthesis-stripped) per the convention documented at the [`Anchor collision covenant` H2 above](#anchor-collision-covenant) — no collision with any prior anchor. Cross-references: see [`Anchor collision covenant` H2 above](#anchor-collision-covenant) for 14th inventory row (parent H2 anchor); the v2.0 / v2.1 / v2.3 / v2.4 commit messages cite the introducer SHAs + commit ranges for each adopted file (see H3s immediately above); the v2.5 commit message (when landed) will reference `audit_script.sh`'s Phase C/G/H grep-pattern additions.
...
The four new H3 slugs `#adopted-tsc-baseline-re-capture-audit-chain-v2-0` / `#adopted-capture-v6-twins-audit-chain-v2-1` / `#deferred-python-docstring-audit-chain-adoption-proposed-v2-2` / `#deferred-linux-portable-ollama-runtime-audit-chain-adoption-proposed-v2-3` all anchor at the parent H2 slug `#future-audit-batch-roadmap` per the forward-extendable convention (no per-H3 explicit-disambiguation sibling needed; distinct from the `### Cross-references` H3 pattern that uses per-Bug explicit-disambiguation siblings `#cross-references-bug-n`). Slugs were generated against the existing GFM semantics (lowercase + spaces→dashes + parenthesis-stripped) per the convention documented at the [`Anchor collision covenant` H2 above](#anchor-collision-covenant) — no collision with any prior anchor, including the original sibling H3 slug `#deferred-host-shell-tooling-audit-chain-adoption`; each H3's bullet list is self-contained. Cross-references: see [`Anchor collision covenant` H2 above](#anchor-collision-covenant) for 14th inventory row (parent H2 anchor); the v2.0 / v2.1 commit messages cite the introducer SHAs + commit ranges for each adopted file (see H3s immediately above).
